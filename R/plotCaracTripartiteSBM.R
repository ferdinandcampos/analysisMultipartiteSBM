...
plotCaracTripartiteSBM=function(caracterisation,
  ...
  values_colors<-rep("#000000",length(analysePlants[[metrique_color]])) #NB si on modifie directement values colors en mettant des codes hexa au milieu des numeric ça pose souci. faire deux objets
  m_values_colors<-min(analysePlants[[metrique_color]])
  M_values_colors<-max(analysePlants[[metrique_color]])
  #Pour forcer la symétrie des couleurs et voir la séparation >0 et <0 #sinon faire deux palettes différentes..
  ...
  #Palette k-means
  if (!study_insectes_only){palette2<-colorRampPalette(brewer.pal(9, "Set1"))(nb_clusters+2)
  }else{palette2<-colorRampPalette(brewer.pal(9, "Set1"))(6+2)
  ...
  ###Affichage multiple, par réseaux
  plot_multiple=TRUE
  if(plot_multiple){
