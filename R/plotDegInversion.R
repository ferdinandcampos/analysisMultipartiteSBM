...
plotDegInversion=function(analyse,
  ...
  for (FG in c("plants","pollination")){
    ...
    # x_lim<-c(-0.4,0.4)
    # y_lim<-c(-0.5,2)
    palette<-grDevices::colorRampPalette(RColorBrewer::brewer.pal(9, "Set1"))(17)
    ...
    plot(data_plants_index[[metrique_x]],data_plants_index[[metrique_y]],col=scales::alpha(palette[1],data_plants_index[["PR_inter1_sym"]]*(data_plants_index[["PR_inter1_sym"]]>seuilPRb_hidecluster)),
    ...
    lines(data_plants_index[[metrique_x]],data_plants_index[[metrique_y]],col=scales::alpha(palette[index],0.3),type="l",lwd=1,lty=1)
    ...
    for (index in Indices[-c(Indices[1])]){
      ...
      #lines(data_plants_index[[metrique_x]],data_plants_index[[metrique_y]],col=scales::alpha(palette[index],data_plants_index[["PR_inter1_sym"]]),pch=17,type=scatter_type,lwd=1,lty=2,cex=exp(data_plants_index[["cluster_pi"]]))
      lines(data_plants_index[[metrique_x]],data_plants_index[[metrique_y]],col=scales::alpha(palette[index],data_plants_index[["PR_inter1_sym"]]*(data_plants_index[["PR_inter1_sym"]]>seuilPRb_hidecluster)),
              cex=exp(data_plants_index[["cluster_pi"]]),pch=17,type="p",lwd=1,lty=1)
      lines(data_plants_index[[metrique_x]],data_plants_index[[metrique_y]],col=scales::alpha(palette[index],0.3),type="l",lwd=1,lty=1)
    }
    if(FG=="plants"){
      legend("topright", legend = c(0.1,0.25,0.5,0.75,1), col = scales::alpha(palette[1],c(0.1,0.25,0.5,0.75,1)),
