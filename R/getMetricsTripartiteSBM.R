...
getMetricsTripartiteSBM=function(myMSBM,normalise=TRUE){
  ...
  metric_degrees[,3:ncol(metric_degrees)]<-lapply(metric_degrees[,3:ncol(metric_degrees)], function(x) as.numeric(x)) #force numerics pour les calculs suivants... #pourquoi ils sont changés en character?
  
  #Calcul des Knn  - Attention dépend ordre colonnes
  for (FG in c(linking_set,inter1,inter2)){
    nb_FG_clust<-myMSBM$nbBlocks[[eval(FG)]]
    if(FG==linking_set){
      for (cluster_id in (1:nb_FG_clust)){
      ...
      for (cluster_id in (1:nb_FG_clust)){
        ...
        if(FG==inter1){ #peut être + propre de faire 3 elsif que des which car ça dépend de l'ordre des colonnes...
  ...
  if (normalise){ #NORMALISATION
    ...
    #Cas limite où un seul block dans un FG
    if (myMSBM$nbBlocks[[eval(inter1)]]==1){ #on force std=1 pour éviter NaN quand (cas K_a = 1)
      cKnn_interactions_std[[1]]<-1 
    }
    if (myMSBM$nbBlocks[[eval(inter2)]]==1){ #on force std=1 pour éviter NaN quand (cas K_b = 1)
      cKnn_interactions_std[[2]]<-1 
    }
    if (myMSBM$nbBlocks[[eval(linking_set)]]==1){ #on force std=1 pour éviter NaN quand (cas K_l = 1)
