library(ggplot2)
library(stats)
library(factoextra) ##ptet le faire à la main ce qu'il fait car bcp de dependencies
...
caracTripartiteSBM=function(analysePlants, 
  ...
  if (weighted_by_pi){
    #Discrétisation/précision au 1%, prendre int(100*pi) fois l'observation a une taille pi
    df<-df[rep(seq_len(nrow(df)),round(100*analysePlants[,"cluster_pi"],0)),]
  }
  
  if (is.null(nb_clusters) & is.null(order_clusters)){ ##Nombre de clusters à déterminer
    x11()
