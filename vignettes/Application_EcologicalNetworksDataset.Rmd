...
```{r plot block caracterisation, eval=TRUE}
plotCaracTripartiteSBM(caracterisation,plotIndicesblocs,new.window=FALSE)
```
...
One can plot the individual blocks' distributions in clusters, per network.
```{r plot block caracterisation distrib, eval=TRUE}
plotCaracTripartiteSBM(caracterisation,plotIndicesblocs,plot.type="distrib.networks",new.window=FALSE)
```
The Multidimensional Scaling (MDS) can also be plotted.
```{r plot block caracterisation mds, eval=TRUE}
plotCaracTripartiteSBM(caracterisation,plotIndicesblocs,plot.type="mds",new.window=FALSE)
```
